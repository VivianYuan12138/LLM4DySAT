...
class LightGCN(nn.Module):
    ...
    def __init__(self, args: dict, datasetloder: BasicDataset, graphs: list, position_embeddings):
        ...
        self.u_att_list = [torch.FloatTensor(dt.u_att).to(args.device) for dt in self.dataset]
        self.m_att_list = [torch.FloatTensor(dt.m_att).to(args.device) for dt in self.dataset]
