...
class StructuralAttentionLayer(nn.Module):
    ...
    def forward(self, graph):
        ...
        result = torch.zeros((graph.x.size(0), x_j.size(-1)), device=x_j.device)
        index = edge_index[1]
        result.index_add_(0, index, (x_j * coefficients[:, :, None]).squeeze(2))
        out = self.act(result)
...
class DySAT(nn.Module):
    def __init__(self,
        ...
        self.loss_graph = []
        # 位置编码
        self.positional_encodings = self.get_positional_encoding(seq_len=self.num_time_steps, d_model=args.latent_dim).to(args.device)
