...
if __name__ == "__main__":
    ...
    name1 = 'train'
    name2 = 'test'
    save_path = r'E:\项目\社交推荐\dataset\movielens 100k\sp_npz\graph.pkl'
    links_3 = np.loadtxt(r'E:\项目\社交推荐\dataset\movielens 100k\mv100k_time.txt', dtype=int, delimiter='\t', encoding='utf-8')
    ...
    all_edge = len(links)
    train_edge = sum([len(Date_list[i]) for i in range(len(Date_list)-2)]) # 训练边数 82073
    val_edge = len(Date_list[-2]) # 13345 验证数量
    ...
    test_edge = len(Date_list[-1]) # 测试的 4582
    ...
    np.savetxt(r'E:\项目\社交推荐\dataset\movielens 100k\\' + 'train_.txt', np.array(Date_list[-3]), fmt="%d", delimiter=',')
    np.savetxt(r'E:\项目\社交推荐\dataset\movielens 100k\\' + 'test_.txt', np.array(Date_list[-1]), fmt="%d", delimiter=',')
    np.savetxt(r'E:\项目\社交推荐\dataset\movielens 100k\\' + 'val_.txt', np.array(Date_list[-2]), fmt="%d", delimiter=',')
