...
from utils.utilsaf import graph_to_dict, graph_to_dict_used_pos
from utils import Procedure
from utils.parse import parse_args
from utils.parse import set_seed
from models.model_mv100k import DySAT
...
if __name__ == "__main__":
    data_path = 'dataset/movielens 100k'
    args = parse_args()
    print(args)
    ...
    val_dict = graph_to_dict(graphs_exchange[-2])
    test_dict = graph_to_dict(graphs_exchange[-1])
    useritem_used_dict = graph_to_dict_used_pos(graphs_exchange[:-2])

    assert args.time_steps <= len(dataset), "Time steps is illegal"  # 判断 步长 和 图是否 对应

    model = DySAT(args=args,
    ...
    for epoch in range(args.epochs):
        ...
        print(f'EPOCH[{epoch + 1}] {loss.cpu().item()}' + str(end - start))
